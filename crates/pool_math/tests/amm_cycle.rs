use pool_math::{DEFAULT_FEE, new_reserves_after_swap, swap_output_amount, verify_swap_invariant};
...
fn integration_full_cycle_small_reserves() {
    ...
    let (new_in, new_out) =
        new_reserves_after_swap(reserve_in, reserve_out, amount_in, DEFAULT_FEE).unwrap();
