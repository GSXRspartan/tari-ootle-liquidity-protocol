...
fn create_faucet(t: &mut TemplateTest, symbol: &str) -> (ComponentAddress, ResourceAddress) {
    ...
    let component: ComponentAddress =
        t.call_function("TestFaucet", "mint_with_symbol", args![initial, symbol.to_string()], vec![]);
    let resource = t
...
fn setup(fee: u16) -> PoolTest {
    let mut t = TemplateTest::new(CRATE_PATH, ["../templates/fungible_pool", "templates/faucet"]);
    ...
    let mut pt = PoolTest { t, a, b, lp, pool, account, account_proof };
...
fn add_liquidity(pt: &mut PoolTest, a_amt: Amount, b_amt: Amount) {
    let (a, b, account, pool, proof) =
        (pt.a, pt.b, pt.account, pt.pool, pt.account_proof.clone());
    let tx = pt
        .t
        .transaction()
...
fn e01_permissionless_add_liquidity_and_lp_received() {
    let mut pt = setup(3);
    add_liquidity(&mut pt, Amount::from(10_000_000u64), Amount::from(10_000_000u64));
    ...
    assert_eq!(lp_held, Amount::from(10_000_000u64) - Amount::from(1_000u64));
}
...
fn e02_redemption_returns_reserves() {
    ...
    assert!(a_ret >= Amount::from(9_990_000u64), "A returned too small (trapped funds?): {}", a_ret);
    assert!(b_ret >= Amount::from(9_990_000u64), "B returned too small (trapped funds?): {}", b_ret);
}
...
fn e03_swap_charges_fee_growing_k() {
    ...
    add_liquidity(&mut pt, Amount::from(1_000_000_000u64), Amount::from(1_000_000_000u64));
    ...
    assert!(k1 > k0, "fee NOT charged: k did not grow (k0={} k1={})", k0, k1);
}
...
fn e04_slippage_min_output_enforced() {
    ...
    let sealed = pt
        ...
        .call_method(pool, "swap", args![Workspace("in"), b, Amount::from(1_000_000_000u64)])
        .put_last_instruction_output_on_workspace("out")
        .call_method(account, "deposit", args![Workspace("out")])
