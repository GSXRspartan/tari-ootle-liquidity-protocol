...
## Native Tari
Native Tari is handled as a fungible resource address (`ResourceAddress`). The TariSwap template treats any fungible resource identically; it does not have special native-Tari mechanics beyond being a fungible resource in the pair.

## Security observations for reuse
1. First-depositor vulnerability exists if tiny initial liquidity allows ratio manipulation.
2. No minimum liquidity lock is enforced.
3. `AccessRules::allow_all()` must be replaced.
4. `ResourceType::Stealth` and `Confidential` are accepted by `check_resource_is_fungible` but AMM math does not model privacy leakage.
5. Fee math rounds down; must verify over many swaps that total fees never exceed theoretical maximum.
